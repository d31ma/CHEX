...
5. Create the PR with `gh pr create` using:
   ...
   - A body with three sections:
     ...
     - **Test plan** — checklist of how to verify the changes (reference `bun test ./tests/index.ts` where relevant).
     - **Breaking changes** — any changes to public API in `src/types/chex.d.ts`; write "None" if there are none.
6. Print the PR URL.
